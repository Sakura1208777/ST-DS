...
def _get_tf(args=None):
    ...
    if _TF is None:
        ...
        _TF = tf
        try:
            _TF.get_logger().setLevel(logging.ERROR)
...
def predictive_score_metrics(ori_data, generated_data, args=None):
    ...
    def predictor(x, t):
        ...
        with tf.compat.v1.variable_scope("predictor", reuse=tf.compat.v1.AUTO_REUSE) as vs:
            ...
            p_outputs, p_last_states = tf.compat.v1.nn.dynamic_rnn(
    ...
    try:
        ...
        for itt in range(iterations):
            ...
            Y_mb = list(
                np.reshape(generated_data[i][1:, (dim - 1)], [len(generated_data[i][1:, (dim - 1)]), 1])
                for i in train_idx
            )
