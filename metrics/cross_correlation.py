...
def cross_correlation_score(ori_data, generated_data, device=None):
    if device is None:
        device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    x_real = torch.as_tensor(ori_data, dtype=torch.float32, device=device)
    x_fake = torch.as_tensor(generated_data, dtype=torch.float32, device=device)
    score = CrossCorrelLoss(x_real=x_real, name='cross_correl')(x_fake)
