...
def _dataloader_kwargs(args, train=True):
    workers = getattr(args, 'num_workers', 0) if train else getattr(args, 'eval_num_workers', getattr(args, 'num_workers', 0))
    ...
    if workers > 0:
        ...
        kwargs['prefetch_factor'] = max(1, int(getattr(args, 'prefetch_factor', 2)))
    return kwargs
...
def gen_dataloader(args):
    if args.dataset == 'sine':
        ...
        train_loader = Data.DataLoader(dataset=train_set, **_dataloader_kwargs(args, train=True))
        test_loader = Data.DataLoader(dataset=test_set, **_dataloader_kwargs(args, train=False))
    ...
    train_loader = Data.DataLoader(dataset=train_set, **_dataloader_kwargs(args, train=True))
    test_loader = Data.DataLoader(dataset=train_set, **_dataloader_kwargs(args, train=False))
