...
class DhariwalUNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, st_film=None):
        # Mapping.
        emb = self.map_noise(noise_labels)
        if self.map_augment is not None and augment_labels is not None:
            emb = emb + self.map_augment(augment_labels)
        ...
        if st_film is not None:
            st_film = st_film.to(device=emb.device, dtype=emb.dtype)
            if st_film.ndim == 1:
                st_film = st_film.unsqueeze(0)
            if st_film.shape[0] == 1 and emb.shape[0] > 1:
                st_film = st_film.expand(emb.shape[0], -1)
            if st_film.shape == emb.shape:
                emb = emb + st_film
        if self.map_label is not None:
            tmp = class_labels
            if self.training and self.label_dropout:
