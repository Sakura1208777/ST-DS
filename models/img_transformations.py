...
class DelayEmbedder(TsImgEmbedder):
    ...
    def ts_to_img(self, signal, pad=True, mask=0):
        ...
        full_cols = max((self.seq_len - self.embedding) // self.delay + 1, 0)
        add_partial = full_cols * self.delay != self.seq_len and full_cols * self.delay + self.embedding > self.seq_len
        cols = full_cols + int(add_partial)

        starts = torch.arange(cols, device=signal.device) * self.delay
        rows = torch.arange(self.embedding, device=signal.device)
        time_idx = starts.unsqueeze(0) + rows.unsqueeze(1)
        valid = time_idx < self.seq_len
        flat_idx = time_idx.clamp(max=max(self.seq_len - 1, 0)).reshape(-1)

        signal_bfl = signal.permute(0, 2, 1)
        gathered = signal_bfl.index_select(dim=2, index=flat_idx)
        x_image = gathered.reshape(batch, features, self.embedding, cols)
        x_image = x_image * valid.reshape(1, 1, self.embedding, cols).to(dtype=signal.dtype)

        # cache the shape of the image before padding
        self.img_shape = (batch, features, self.embedding, cols)
    ...
    def img_to_ts(self, img):
        ...
        time = torch.arange(self.seq_len, device=img.device)
        starts = torch.arange(cols, device=img.device) * self.delay
        valid = (starts.unsqueeze(1) <= time.unsqueeze(0)) & (
            time.unsqueeze(0) < starts.unsqueeze(1) + rows
        )
        col_candidates = torch.arange(cols, device=img.device).unsqueeze(1).expand(cols, self.seq_len)
        chosen_cols = torch.where(valid, col_candidates, col_candidates.new_full((), -1)).max(dim=0).values
        valid_time = chosen_cols >= 0
        safe_cols = chosen_cols.clamp_min(0)
        chosen_rows = (time - starts.index_select(0, safe_cols)).clamp(0, max(rows - 1, 0))
        flat_idx = chosen_rows * cols + safe_cols

        img_flat = img_non_square.reshape(batch, channels, rows * cols)
        reconstructed_x_time_series = img_flat.index_select(dim=2, index=flat_idx)
        reconstructed_x_time_series = reconstructed_x_time_series * valid_time.reshape(1, 1, self.seq_len).to(
            dtype=img.dtype
        )
